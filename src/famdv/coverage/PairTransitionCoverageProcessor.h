...
#ifndef FAM_COVERAGE_PAIR_TRANSITION_PROCESSOR_H_
#define FAM_COVERAGE_PAIR_TRANSITION_PROCESSOR_H_
...
class PairTransitionCoverageProcessor final :
		public AutoRegisteredCoverageProcessorUnit< PairTransitionCoverageProcessor >
{

	AVM_DECLARE_CLONABLE_CLASS( PairTransitionCoverageProcessor )
	...
	AVM_INJECT_AUTO_REGISTER_QUALIFIED_ID_KEY_2(
			"coverage#pair#transition",
			"avm::processor.PAIR_TRANSITION_COVERAGE")
	// end registration
...
protected:
	...
	AvmCoverageStatistics mTransitionCoverageStatistics;
	...
	Array2OfBitset * mExecutablePairCoverageTable;

	ArrayOfBitset * mTransitionCoverageTable;
	Array2OfBitset * mTransitionPairCoverageTable;
...
public:
	 ...
	 * Default
	 */
	PairTransitionCoverageProcessor(
	...
	mTransitionCoverageStatistics( 100 , 0 ),

	mExecutableCoverageTable( nullptr ),
	mExecutablePairCoverageTable( nullptr ),

	mTransitionCoverageTable( nullptr ),
	mTransitionPairCoverageTable( nullptr ),
	 ...
	 * DESTRUCTOR
	 */
	virtual ~PairTransitionCoverageProcessor();
	...
	static void reportCoverageStatePairTransitions(OutStream & os,
			ArrayOfBitset * coverageTable, const ExecutableForm & anExecutable,
			const std::string & stateID, const std::string & nature);

	void reportUncoveredPairTransition(OutStream & os) const;
	void reportUncoveredTransition(OutStream & os) const;
	...
	{
		// SET EXIT CODE
		mTransitionCoverageStatistics.setExitCode();

		return BaseCoverageFilter::exitImpl();
	}
	/**
	 * postEval Filter
	 */
	...
	void updateTransitionCoverageTable(ExecutionContext * anEC,
			const RuntimeID & aRID, AvmTransition * previousTransition,
...
#endif /* FAM_COVERAGE_PAIR_TRANSITION_PROCESSOR_H_ */
