...
#include "PairTransitionCoverageProcessor.h"
 ...
 * DESTRUCTOR
 */
PairTransitionCoverageProcessor::~PairTransitionCoverageProcessor()
{
...
bool PairTransitionCoverageProcessor::configureImpl()
{
	// SUPER CONFIGURATION
...
AVM_IF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
	...
	AVM_OS_TRACE << "Uncovered Pair-Transition Count at the beginning :> "
			<< mCoverageStatistics.mNumberOfElements << std::endl << std::endl;
AVM_ENDIF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
	}

//	mConfigFlag = getExecutionQueue().reconfigure(ExecutionQueue::STRATEGY_BFS);
...
void PairTransitionCoverageProcessor::configureExecutableCoverageTableFlag(bool value)
{
	if( mExecutableCoverageTable == nullptr )
	{
		...
		mExecutablePairCoverageTable = new Array2OfBitset(
				getConfiguration().getExecutableSystem().size(), nullptr);
	}

	Bitset * tableOfFlag = nullptr;
	ArrayOfBitset * tableOfPairFlag = nullptr;
...
AVM_ENDIF_DEBUG_LEVEL_FLAG_AND( MEDIUM , CONFIGURING )
			...
			tableOfPairFlag = mExecutablePairCoverageTable->at(itExec->getOffset());
			if( tableOfPairFlag == nullptr )
			{
				tableOfPairFlag = new ArrayOfBitset(itExec->getTransition().size());

				mExecutablePairCoverageTable->set(
						itExec->getOffset(), tableOfPairFlag);
			}
			...
			for( itTransition = 0 ; itTransition < endTransition ; ++itTransition )
			{
				if( (not value) && tableOfFlag->test(itTransition) )
				{
					...
					mTransitionCoverageStatistics.addUncoveredElement();
...
AVM_ENDIF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
				...
				const AvmTransition * avmTransition =
						itExec->rawTransition(itTransition);
				const ExecutableForm * targetExecState =
						avmTransition->getTargetMarchine()->getExecutable();
				if( targetExecState->hasTransition() )
				{
					Bitset * table2OfFlag = new Bitset(
							targetExecState->getTransition().size(), false);
					tableOfPairFlag->set(itTransition, table2OfFlag);

					mCoverageStatistics.addUncoveredElement(
							targetExecState->getTransition().size());
				}
			}
...
AVM_ENDIF_DEBUG_LEVEL_FLAG_AND( MEDIUM , CONFIGURING )
		...
		else
		{
			...
			mExecutablePairCoverageTable->set(itExec->getOffset(), nullptr);
		}
	}
}


void PairTransitionCoverageProcessor::configureExecutableCoverageTableFlag(
...
AVM_ENDIF_DEBUG_LEVEL_FLAG_AND( MEDIUM , CONFIGURING )
		...
		ArrayOfBitset * tableOfPairFlag = mExecutablePairCoverageTable->at(anExecutable.getOffset());
		if( tableOfPairFlag == nullptr )
		{
			tableOfPairFlag = new ArrayOfBitset(anExecutable.getTransition().size());

			mExecutablePairCoverageTable->set(
					anExecutable.getOffset(), tableOfPairFlag);
		}
...
AVM_ENDIF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
			...
			const AvmTransition * avmTransition =
					anExecutable.rawTransition(itTransition);
			const ExecutableForm * targetExecState =
					avmTransition->getTargetMarchine()->getExecutable();
			if( targetExecState->hasTransition() )
			{
...
void PairTransitionCoverageProcessor::configureInstanceCoverageTableFlag()
{
	if( mExecutableCoverageTable != nullptr)
	{
		const ExecutionData & anED = getConfiguration().getMainExecutionData();
		...
		if( mTransitionCoverageTable == nullptr )
		{
			...
			mTransitionPairCoverageTable = new Array2OfBitset(
					anED.getTableOfRuntime().size(), nullptr);
		}
		...
		for( std::size_t itMachine = 0 ; itMachine < endMachine ; ++itMachine )
		{
					...
					(mTransitionCoverageTable->at(itMachine) == nullptr) )
			{
				...
				mTransitionPairCoverageTable->set(itMachine,
						mExecutablePairCoverageTable->at(
								aRID.refExecutable().getOffset()));
			}
		}
	}
...
void PairTransitionCoverageProcessor::configureInstanceCoverageTableFlag(bool value)
{
	const ExecutionData & anED = getConfiguration().getMainExecutionData();
	RuntimeID aRID;
	Bitset * tableOfFlag = nullptr;
...
AVM_ENDIF_DEBUG_LEVEL_FLAG_AND( MEDIUM , CONFIGURING )
			...
			tableOfPairFlag = mExecutablePairCoverageTable->at(
					aRID.refExecutable().getOffset());
			if( tableOfPairFlag == nullptr )
			{
				tableOfPairFlag = new ArrayOfBitset(
						aRID.refExecutable().getTransition().size());

				mTransitionPairCoverageTable->set(itMachine, tableOfPairFlag);
			}
...
AVM_ENDIF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
				...
				const AvmTransition * avmTransition =
						aRID.refExecutable().rawTransition(itTransition);
				const ExecutableForm * targetExecState =
						avmTransition->getTargetMarchine()->getExecutable();
				if( targetExecState->hasTransition() )
				{
...
AVM_ENDIF_DEBUG_LEVEL_FLAG_AND( MEDIUM , CONFIGURING )
		...
		else
		{
			...
			mTransitionPairCoverageTable->set(itMachine, nullptr);
		}
	}
}
...
AVM_ENDIF_DEBUG_LEVEL_FLAG_AND( MEDIUM , CONFIGURING )
			...
			ArrayOfBitset * tableOfPairFlag = mExecutablePairCoverageTable->at(
					aRID.refExecutable().getOffset());
			if( tableOfPairFlag == nullptr )
			{
				tableOfPairFlag = new ArrayOfBitset(
				...
				mTransitionPairCoverageTable->set(aRID.getOffset(), tableOfPairFlag);
			}
...
void PairTransitionCoverageProcessor::configureTransitionCoverageTableFlag(
...
Bitset * PairTransitionCoverageProcessor::getCoverageTableBitset(
 ...
 * REPORT
 */
void PairTransitionCoverageProcessor::reportMinimum(OutStream & os) const
{
	os << TAB << "PAIR-TRANSITION COVERAGE PROCESSOR" << EOL_FLUSH;

	if( mCoverageStatistics.goalAchieved() )
	{
		os << TAB2 << "All the << " << mCoverageStatistics.mNumberOfElements
				<< " >> pair-transitions are covered !" << EOL;
	}
	else
	{
		os << TAB2 << "Warning: all the pair-transitions are not covered !" << EOL;
		mCoverageStatistics.toStreamCoverageRate( os << TAB2,
				"Results: << ", " on "," >> pair-transitions are covered !\n" );

		if( mTransitionCoverageStatistics.goalAchieved() )
		{
			os << TAB2 << "But all the << "
					<< mTransitionCoverageStatistics.mNumberOfElements
					<< " >> transitions are covered !" << EOL;
		}
		else
		{
			mTransitionCoverageStatistics.toStreamCoverageRate( os << TAB2,
							"Results: << ", " on "," >> transitions are covered !\n" );
		}

		if( isReportDetails() )
		{
			if( not mTransitionCoverageStatistics.goalAchieved() )
			{
				reportUncoveredTransition(os);
			}
			reportUncoveredPairTransition(os);
		}
...
void PairTransitionCoverageProcessor::reportCoverageStatePairTransitions(OutStream & os,
		ArrayOfBitset * pairCoverageTable, const ExecutableForm & anExecutable,
		const std::string & firstStateID, const std::string & nature)
{
	Bitset * coverageTable = nullptr;
	std::size_t itTransition = 0;
	std::size_t endTransition = anExecutable.getTransition().size();
	for( ; itTransition < endTransition ; ++itTransition )
	{
		coverageTable = pairCoverageTable->at(itTransition);

		std::size_t uncoveredCount = (coverageTable == nullptr) ? 0
				: coverageTable->size() - coverageTable->count();
//os << TAB3 << "transitions : uncovered count = " << uncoveredCount << EOL_FLUSH;

		if( uncoveredCount > 0 )
		{
			if( not anExecutable.isReachableState() )
			{
				os << TAB3 << nature << "[ NO INCOMING TRANSITION ] :> " << firstStateID << EOL;
			}

			const AvmTransition * firstTransition =
					anExecutable.rawTransition(itTransition);
			const ExecutableForm * targetExecState =
					firstTransition->getTargetMarchine()->getExecutable();
			const std::string & secondStateID = targetExecState->getNameID();

			AvmTransition * secondTransition = nullptr;
			std::size_t endOffset = targetExecState->getTransition().size();

			os << TAB3 << "Pair-transitions with " << firstStateID << " --> "
					<< firstTransition->getNameID() << " --> "
					<< secondStateID  << " {" << EOL;
			for( std::size_t offset = 0 ; offset < endOffset ; ++offset )
			{
				if( not coverageTable->test( offset ) )
				{
					secondTransition = targetExecState->getTransition().rawAt(offset);

					os << TAB4 << secondTransition->getNameID() << " --> "
							<< secondTransition->getTarget().str() << EOL;
				}
			}
			os << TAB3 << "}" << EOL;
		}
...
void PairTransitionCoverageProcessor::reportUncoveredPairTransition(OutStream & os) const
{
	os << TAB2 << "List of the << "
			<< mCoverageStatistics.getNumberOfUncovered()
			<<  " >> pair-transitions non covered:" << EOL << EOL;
	...
	switch( mScope )
	{
		case Specifier::DESIGN_INSTANCE_KIND:
		{
			if( mTransitionPairCoverageTable == nullptr )
			{
				break;
			}

			std::size_t endMachine = mTransitionPairCoverageTable->size();
			std::size_t itMachine = 0;
			for( ; itMachine < endMachine ; ++itMachine )
			{
				const RuntimeID & aRID = anED.getRuntimeID(itMachine);

				reportCoverageStatePairTransitions(os,
						mTransitionPairCoverageTable->at(aRID.getOffset()),
						aRID.refExecutable(), aRID.getQualifiedNameID(), "instance");
			}
		...
		case Specifier::DESIGN_MODEL_KIND:
		{
			if( mExecutablePairCoverageTable == nullptr )
			{
				break;
			}
					...
					++itExec , ++indexExec )
			{
				...
				reportCoverageStatePairTransitions(os,
						mExecutablePairCoverageTable->at(anExecutable.getOffset()),
						anExecutable, anExecutable.getQualifiedNameID(), "model");
			}
...
void PairTransitionCoverageProcessor::reportUncoveredTransition(OutStream & os) const
{
	os << TAB2 << "List of the << "
			<< mTransitionCoverageStatistics.getNumberOfUncovered()
...
void PairTransitionCoverageProcessor::reportDefault(OutStream & os) const
{
	reportHeader(os, "PAIR-TRANSITION COVERAGE ");
...
void PairTransitionCoverageProcessor::tddRegressionReportImpl(OutStream & os) const
{
	os << TAB1 << "COVERED PAIR-TRANSITIONS : ";
	if( mCoverageStatistics.goalAchieved() )
	{
 ...
 * PRE-FILTER
 */
bool PairTransitionCoverageProcessor::prefilter()
{
//	bool mCondition =
 ...
 * postEval Filter
 */
bool PairTransitionCoverageProcessor::postfilter()
{
 ...
 */
void PairTransitionCoverageProcessor::heuristicNaiveClassImpl()
{
	if( mCoverageStatistics.hasUncovered() )
	{
AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
 ...
 */
void PairTransitionCoverageProcessor::heuristicSmartClassImpl()
{
	heuristicNaiveClassImpl();
}
 ...
 */
void PairTransitionCoverageProcessor::heuristicAgressiveClassImpl()
{
	heuristicNaiveClassImpl();
}
 ...
 */
void PairTransitionCoverageProcessor::heuristicNothingElseClassImpl()
{
 ...
 */
void PairTransitionCoverageProcessor::setHitWeight(VectorOfExecutionContext & hitEC,
 ...
 * weight: 3  else
 */
void PairTransitionCoverageProcessor::computeWeight(ExecutionContext * anEC)
{
	switch( mHeuristicProperty.mHeuristicClass )
	{
		case IHeuristicClass::HEURISTIC_BASIC_CLASS:
...
void PairTransitionCoverageProcessor::computeWeightNaive(ExecutionContext * anEC)
{
...
void PairTransitionCoverageProcessor::computeWeightSmart(ExecutionContext * anEC)
{
	computePriorityWeight(anEC);
...
void PairTransitionCoverageProcessor::computeWeightAgressive(ExecutionContext * anEC)
{
	computePriorityWeight(anEC);
...
bool PairTransitionCoverageProcessor::computeCheckNonPriorityWeight(
...
void PairTransitionCoverageProcessor::computePriorityWeight(ExecutionContext * anEC)
{
...
bool PairTransitionCoverageProcessor::checkStronglyPriorityWeight(
		ExecutionContext * anEC)
{
...
bool PairTransitionCoverageProcessor::checkWeaklyPriorityWeight(ExecutionContext * anEC)
{
...
void PairTransitionCoverageProcessor::computeWeightOfResult()
{
	mStronglyHitEC.clear();
...
void PairTransitionCoverageProcessor::handleRequestRequeueWaitingTable(
...
bool PairTransitionCoverageProcessor::testFireability(const ExecutionData & anED,
...
bool PairTransitionCoverageProcessor::isControlLoop(ExecutionContext * anEC)
{
...
bool PairTransitionCoverageProcessor::isSyntaxicLoop(ExecutionContext * anEC)
{
	tmpEC = anEC->getPrevious();
	for( ; tmpEC != nullptr ; tmpEC = tmpEC->getPrevious() )
	{
...
bool PairTransitionCoverageProcessor::isTrivialLoop(ExecutionContext * anEC)
{
	tmpEC = anEC->getPrevious();
	for( ; tmpEC != nullptr ; tmpEC = tmpEC->getPrevious() )
	{
...
void PairTransitionCoverageProcessor::fireableTransitionCount(
...
void PairTransitionCoverageProcessor::fireableTransitionTrace(
...
void PairTransitionCoverageProcessor::fireableTransitionTrace(OutStream & os,
		const ExecutionData & anED, const RuntimeForm & aRF)
{
	tmpEF = aRF.getRID().getExecutable();
 ...
 */
AvmTransition * getPreviousTransition(ExecutionContext * anEC, const BF & aFiredCode)
{
	...
	switch( aFiredCode.classKind() )
	{
		case FORM_AVMCODE_KIND:
		{
			...
			{
				AvmTransition * prevTransition =
						getPreviousTransition(anEC, itOperand);
				if( prevTransition != nullptr )
				{
		...
		case FORM_EXECUTION_CONFIGURATION_KIND:
		{
			...
			{
				return anExecConf.getTransition();
			}
...
void PairTransitionCoverageProcessor::updateTransitionCoverageTable(
		ExecutionContext * anEC, const BF & aFiredCode)
{
	...
	switch( aFiredCode.classKind() )
	{
		...
		case FORM_EXECUTION_CONFIGURATION_KIND:
		{
			...
			if( anExecConf.isTransition() )
			{
				AvmTransition * previousTransition =
						getPreviousTransition(anEC, anEC->getPrevious()->getRunnableElementTrace());

				updateTransitionCoverageTable(anEC, anExecConf.getRuntimeID(),
						previousTransition, anExecConf.getTransition());
			}
...
void PairTransitionCoverageProcessor::updateTransitionCoverageTable(
		...
		AvmTransition * previousTransition, AvmTransition * firedTransition)
{
	bool isNewlyCovered = false;
	bool isNewlyPairCovered = false;

	avm_offset_t previousStateOffset = ( previousTransition == nullptr ) ? 0 :
			previousTransition->getTransitionSource()->getOffset();
	avm_offset_t prevTransitionOffset = ( previousTransition == nullptr ) ? 0 :
			previousTransition->getOffset();
	avm_offset_t firedTransitionOffset = firedTransition->getOffset();

	if( previousTransition != nullptr )
	{
AVM_OS_DEBUG << "pair-transition : " << previousTransition->getFQNameID()
	<< " , " << firedTransition->getFQNameID() << std::endl;
	}
	...
	switch( mScope )
	{
		case Specifier::DESIGN_MODEL_KIND:
		{
			...
			if( previousTransition != nullptr )
			{
				ArrayOfBitset * tableOfPairFlag =
						mExecutablePairCoverageTable->at(previousStateOffset);
				if( not tableOfPairFlag->at(prevTransitionOffset)->test(firedTransitionOffset) )
				{
					isNewlyPairCovered = true;

					tableOfPairFlag->at(prevTransitionOffset)->set(firedTransitionOffset, true);
				}
			}
		...
		case Specifier::DESIGN_INSTANCE_KIND:
		{
			...
			{
				ArrayOfBitset * tableOfPairFlag =
						mTransitionPairCoverageTable->at(previousStateOffset);
				if( not tableOfPairFlag->at(prevTransitionOffset)->test(firedTransitionOffset) )
				{
					isNewlyPairCovered = true;
		...
		default:
		{
			...
			if( previousTransition != nullptr )
			{
				...
				if( not tableOfPairFlag->at(prevTransitionOffset)->test(firedTransitionOffset) )
				{
					...
					if( mExecutableCoverageTable != nullptr )
					{
						tableOfPairFlag = mExecutablePairCoverageTable->at(previousStateOffset);
						tableOfPairFlag->at(prevTransitionOffset)->set(firedTransitionOffset, true);
					}
				}
			}
	...
	if( isNewlyPairCovered )
	{
		mCoverageStatistics.addCoveredElement();

		anEC->getwFlags().addCoverageElementTrace();

		anEC->addInfo( *this, INCR_BF( previousTransition ) );
		anEC->addInfo( *this, INCR_BF( firedTransition ) );
	}

	if( isNewlyCovered )
	{
		mTransitionCoverageStatistics.addCoveredElement();
	}
}


bool PairTransitionCoverageProcessor::testTransitionCoverage(
 ...
 */
void PairTransitionCoverageProcessor::collectUncoveredTransition()
{
...
AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	AVM_OS_INFO << "PairTransitionCoverageProcessor::collectUncoveredTransition:> count: "
 ...
 * mTraceDriver
 */
bool PairTransitionCoverageProcessor::initializeTraceDriver(
		WaitingStrategy & aWaitingStrategy)
{
...
bool PairTransitionCoverageProcessor::initializeTraceDriver(ExecutionContext * anEC)
{
...
bool PairTransitionCoverageProcessor::runTraceDriver()
{
