 ...
 */
void AvmTraceProperty::checkDecrementation(const AvmTransition * aTransition)
{
	if( (mTransitionCoverage != nullptr) &&
...
void AvmTraceProperty::checkIncrementation(const AvmTransition * aTransition)
{
	if( (mTransitionCoverage != nullptr) &&
		(not mTransitionCoverage->testTransitionCoverage(aTransition)) )
	{
...
void AvmTraceProperty::updadeSize()
{
	...
	for( const auto & itPoint : points )
	{
		...
		checkIncrementation(
				itPoint.to< TracePoint >().object->as_ptr< AvmTransition >() );
	}
}


bool AvmTraceProperty::compare( bool (* isComp)(std::size_t, std::size_t),
...
void AvmTraceProperty::toStream(OutStream & os, bool verbose) const
{
	...
	{
		os << INCR_INDENT;
		const AvmTransition * aTransition = nullptr;
		for( const auto & itPoint : points )
		{
			aTransition = itPoint.to< TracePoint >().
					object->as_ptr< AvmTransition >();
...
mEC( anEC ),
mED( anEC->getExecutionData() ),
mRID( tpTransition->RID ),

mTransition( tpTransition->object->to_ptr< AvmTransition >() ),
...
//mTransitionPath( ),
//mExpectedInput( ),
mVirtualBuffer( const_cast< AvmTransition & >(* mTransition),
...
bool AvmCoverageDirectiveTraceBuilder::initialize()
{
	...
	if( mRID.invalid() )
	{
		if( mED.getRID().refExecutable().getTransition().contains(mTransition) )
		{
			mRID = mED.getRID();
		}
		...
		if( mRID.invalid() )
		{
			...
			{
				mRID = mED.getRuntimeID( mTransition->refExecutable() );
			}
		}
	}
...
void AvmCoverageDirectiveTraceBuilder::report(OutStream & os)
{
	...
	mTraceElement.toStream(os, false);

//	os << "Expected Input :> " << std::endl;
//	ObjectElement::toStreamStaticCom(os, mExpectedInput);
...
AVM_IF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
	...
	if( aTransition->hasInternalCommunicationCode() )
	{
		...
		ObjectElement::toStreamStaticCom(AVM_OS_TRACE,
				aTransition->getInternalCommunicationCode());
	}
//	AVM_OS_TRACE << TAB << "Expected Input :> " << std::endl;
//	ObjectElement::toStreamStaticCom(AVM_OS_TRACE, mExpectedInput);
...
AVM_ENDIF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
	...
	std::size_t saveTraceElementSize = mTraceElement.mSize;
...
AVM_IF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
	...
	AVM_OS_TRACE << TAB2 << "Expected Input :> "
...
AVM_ENDIF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
	...
	for( const auto & itRF : mED.getTableOfRuntime() )
	{
		srcRID = itRF->getRID();
...
AVM_IF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
	AVM_OS_TRACE << TAB << "Unfound Input :> "
			<< anInputTrace->getFullyQualifiedNameID() << std::endl;
AVM_ENDIF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
		...
		default:
		{
			if( StatementTypeChecker::isSchedule(* aCode) )
			{
				for( const auto & itOperand : aCode->getOperands() )
				{
					...
					{
						computeTargetMachine(aRID,
								itOperand.to_ptr< AvmCode >());
					}
				}
			}
			break;
