...
mMakeTrivialAssigmentFromGuardEnabled( false ),
mMakeTrivialAssigmentFromTimedGuardEnabled( false ),
		...
		if( configureFormatter(aSECTION, mOutputFilenamePattern, "filename" ) )
		{
			...
			if( mOutputExecutableEnabledGenerationFlag )
			{
				mOutputExecutableFileLocation = VFS::native_path(
					mOutputExecutableFileLocation, VFS::WorkspaceDebugPath);
			}
			else
			{
				mOutputExecutableFileLocation =
			...
			if( mOutputInitializationEnabledGenerationFlag )
			{
				mOutputInitializationFileLocation = VFS::native_path(
					mOutputInitializationFileLocation, VFS::WorkspaceDebugPath);
			}
			else
			{
				mOutputInitializationFileLocation =
			...
			if( mOutputSymbexGraphEnabledGenerationFlag )
			{
				mOutputSymbexGraphFileLocation = VFS::native_path(
					mOutputSymbexGraphFileLocation, VFS::WorkspaceDebugPath);
			}
			else
			{
...
bool Configuration::configureSymbexImpl(const WObject * wfSequenceSYMBEX)
{
	...
	mPathConditionDisjonctionSeparationEnabled =
			Query::getWPropertyBoolean(wfSequenceSYMBEX,
					CONS_WID4("separation", "of", "pc", "disjunction"),
					mPathConditionDisjonctionSeparationEnabled);

	mMakeTrivialAssigmentFromGuardEnabled =
			Query::getWPropertyBoolean(wfSequenceSYMBEX,
					CONS_WID5("make", "trivial", "assignment", "from", "guard"),
					mMakeTrivialAssigmentFromGuardEnabled);

	mMakeTrivialAssigmentFromTimedGuardEnabled =
			Query::getWPropertyBoolean(wfSequenceSYMBEX,
					CONS_WID6("make", "trivial", "assignment", "from", "timed", "guard"),
					mMakeTrivialAssigmentFromTimedGuardEnabled);
	...
	SolverDef::DEFAULT_SOLVER_KIND =
			SolverDef::toSolver(solverKind, SolverDef::SOLVER_Z3_KIND);
...
void Configuration::setActive() const
{
	...
	AvmcodeGuardCompiler::MAKE_TRIVIAL_ASSIGMENT_FROM_GUARD_ENABLED =
			mMakeTrivialAssigmentFromGuardEnabled;

	AvmcodeTimedGuardCompiler::MAKE_TRIVIAL_ASSIGMENT_FROM_TIMED_GUARD_ENABLED =
			mMakeTrivialAssigmentFromTimedGuardEnabled;
}
...
void Configuration::serializeScenarii() const
{
	...
	if( resultStream.good() )
	{
		...
		for( const auto & itEC : getExecutionTrace() )
		{
			itEC->toFscn(out, ExecutionData::nullref());
		}
