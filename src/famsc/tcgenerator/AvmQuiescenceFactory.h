...
public:
	 ...
	 * Default
	 */
	AvmQuiescenceFactory(AvmPathGuidedTestcaseGenerator & aProcessor);
	...
	bool recSimplifyQuiescenceOf(ExecutionContext & anEC);
	bool simplifyQuiescenceOf(ExecutionContext & anEC);
	bool simplifyAdmissibleQuiescenceOf(ExecutionContext & anEC);
	bool simplifyOutputQuiescenceOf(ExecutionContext & anEC);

	BF computeOutputQuiescencePathCondition(const ExecutionContext & tcSourceEC);
	BF computeAdmissibleQuiescencePathCondition(const ExecutionContext & tcSourceEC);

	BF computeOutputQuiescenceNodeCondition(const ExecutionContext & tcSourceEC);
	BF computeAdmissibleQuiescenceNodeCondition(const ExecutionContext & tcSourceEC);
