...
//AVM_OS_COUT << "Expression :> " << operands.last().str() << std::endl;
//for( const auto & itVar : listOfVar )
//{
//	AVM_OS_COUT << "\texprVar :> " << itVar.str() << std::endl;
//}

//	auto comparer = [](const BF & v_1, const BF & v_2) -> bool {
//		return( v_1.str() == v_2.str() );
//	};
	...
	for( const auto & itOperand : operands )
	{
		...
		{
//AVM_OS_COUT << "boundVar :> " << itOperand.str() << std::endl;
...
//AVM_OS_COUT << "\texprVar :> " << itVar.str() << std::endl;
				if( NamedElement::extractNameID(itOperand.str()) ==
						NamedElement::extractNameID(itVar.str()) )
				{
//AVM_OS_COUT << "found :> " << itOperand.str() << std::endl;
					selectedOperands.append(itOperand);
					break;
				}
			}

//			if( std::find_if(listOfVar.begin(), listOfVar.end(), comparer) != listOfVar.end() )
////			if( listOfVar.contains(itOperand) )
//			{
//				selectedOperands.append(itOperand);
//			}
		}
	...
	{
//AVM_OS_COUT << "Result :> " << operands.last().str() << std::endl;
...
//			}
			...
			{
				BFCode dupExpr(OperatorManager::OPERATOR_FORALL);
				dupExpr.getOperands().append(aCode.getOperands());
				BF predicat = dupExpr.getOperands().pop_last();
				dupExpr.append( notExpr(predicat) );
				return( dupExpr );
			}

			case AVM_OPCODE_FORALL:
			{
				BFCode dupExpr(OperatorManager::OPERATOR_EXISTS);
				dupExpr.getOperands().append(aCode.getOperands());
				BF predicat = dupExpr.getOperands().pop_last();
				dupExpr.append( notExpr(predicat) );
				return( dupExpr );
