...
void ExpressionFactory::collectsAnyFreeVariable(const BF & anExpr,
		BFCollection & listOfBoundVar, BFCollection & listOfVar)
{
	switch( anExpr.classKind() )
	{
		case FORM_AVMCODE_KIND:
		{
			collectsAnyFreeVariable(anExpr.bfCode(), listOfBoundVar, listOfVar);
...
void ExpressionFactory::collectsAnyFreeVariable(const BFCode & aCode,
		BFCollection & listOfBoundVar, BFCollection & listOfVar)
{
	switch( aCode.getAvmOpCode() )
	{
		...
		case AVM_OPCODE_FORALL:
		{
			...
			InstanceOfData::Table occursVars;

			collectsAnyFreeVariable(aCode->last(), listOfBoundVar, occursVars);

			occursVars.remove(boundVars);
			listOfVar.add_unique(occursVars);
			break;
		}
		default:
		{
			for( const auto & itOperand : aCode.getOperands() )
			{
				collectsAnyFreeVariable(itOperand, listOfBoundVar, listOfVar);
			}
			break;
		}
	}
		...
		InstanceOfData::Table & listOfBoundVar, InstanceOfData::Table & listOfVar)
{
	switch( aCode.getAvmOpCode() )
	{
		...
		case AVM_OPCODE_FORALL:
		{
			...
			collectsFreeVariable(aCode->last(), listOfBoundVar, occursVars);
