...
public:
	 ...
	 */
	inline virtual const std::string & getFullyQualifiedNameID() const override final
	{
...
AVM_ENDIF_DEBUG_FLAG( QUALIFIED_NAME_ID )
	 ...
	 * mNameID
	 */
	inline virtual const std::string & getNameID() const override final
	{
		if( mNameID.empty() )
		{
			//? updateNameID();
	...
	virtual std::string strHeader() const override final
	{
		StringOutStream oss;
