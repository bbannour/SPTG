...
inline void WRAP_TAB(OutStream & os)
{
	bool isnotWrap = true;
	if( not os.INDENT.EOL.starts_with("\n") )
	{
		std::stack< AvmIndent > stackClone(os.STACK);
		while( not stackClone.empty() )
		{
			AvmIndent indent = stackClone.top();
			stackClone.pop();
			if( indent.EOL.starts_with("\n") )
			{
				os << indent.EOL << indent.TABS;
				isnotWrap = false;
				break;
			}
		}
...
inline void WRAP_TAB2(OutStream & os)
{
	bool isnotWrap = true;
	if( not os.INDENT.EOL.starts_with("\n") )
	{
		if( not os.INDENT.EOL.starts_with("\n") )
		{
			...
			while( not stackClone.empty() )
			{
				...
				if( indent.EOL.starts_with("\n") )
				{
					os << indent.EOL << indent.TABS << indent.CHAR;
					isnotWrap = false;
					break;
				}
			}
