...
class FormulaData final :  public Element ,
...
public:
	 ...
	 * Default
	 */
	FormulaCoverageFilter(SymbexControllerUnitManager & aManager,
	...
	BF appendFormula(const BF & aFormula,
			const std::string & label, Element * favmFormula = nullptr);
	...
	inline BF & getFormula(const std::string & label)
	{
		...
		{
			if( (*itForm).to< FormulaData >().label == label )
			{
				return( *itForm );
			}
		}
...
class FormulaCoverageFilterInfo : public Element ,
...
public:
	 ...
	 * mMocTable
	 */
	inline void appendMoc(const BF & aFD)
	{
