...
{
	for( std::size_t offset = 0 ; offset < mTraceObjective.size() ; ++offset )
	{
...
bool BaseHitProcessor::configure(const WObject * wfParameterObject)
{
...
bool BaseHitProcessor::isAbsoluteLeaf(ExecutionContext & anEC)
{
	if( not anEC.hasNext() )
	{
		if( mRelativeLeafEC.contains(& anEC) )
		{
			return( false );
		}
		else if( anEC.getFlags().hasReachedLimit() )
		{
			mRelativeLeafEC.append(& anEC);

			return false;
		}
		else if( (anEC.getHeight() < mJumpHeight)
					&& (mJumpHeight < AVM_NUMERIC_MAX_SIZE_T) )
		{
