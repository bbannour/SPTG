		...
		VectorOfInt & aLinkTable, const std::string & title)
{
	...
	std::size_t endIdx = aFormulaTable.size();
	for( std::size_t idx = 0; idx < endIdx ; ++idx )
	{
		os << "\t" << aLinkTable[idx] << ":> "
				<< aFormulaTable[idx].to< FormulaData >().label << "\n"
				<< std::flush;
	}
}
		...
		const std::string & title)
{
	...
	for( std::size_t idx = 0; idx < endIdx ; ++idx )
	{
		if( aFormulaTable[idx].to< FormulaData >().satEC.nonempty() )
		{
			aFormulaTable[idx].to< FormulaData >().report(os);
		}
	}
}
...
bool FormulaCoverageFilter::configureImpl()
{
	...
	if( theDATA != WObject::_NULL_ )
	{
		...
		List< WObject * >::const_iterator it = listOfLocalVar.begin();
		List< WObject * >::const_iterator itEnd = listOfLocalVar.end();
...
//!![MIGRATION]
			BF anInstance( new InstanceOfData(
					...
					&mLocalExecutableForm, /*(*it)*/ Variable::nullref(),
					aTypeSpecifier, offset) );
	...
	const WObject * theFORMULA = Query::getRegexWSequence(
...
AVM_ENDIF_DEBUG_FLAG( CONFIGURING )
	...
	const WObject * theMOC = Query::getRegexWSequence(
...
//					->getExecutionData(), mLocalExecutableForm, aFormula);
...
BF FormulaCoverageFilter::buildMoc(const BF & aMoc)
{
	switch( aMoc.classKind() )
	{
		...
		{
			BFCode newMoc( aMoc.to< AvmCode >().getOperator() );

			BF bfMoc;

			for( const auto & itOperand : aMoc.to< AvmCode >().getOperands() )
			{
				if( (bfMoc = buildMoc(itOperand)).valid() )
				{
					newMoc->append( bfMoc );
				}
				else
...
void FormulaData::report(OutStream & os) const
{
	...
	for( const auto & itEC : satEC )
	{
		os << TAB2 << "==>E[?] " << itEC->str_min() << EOL;
	}
	os << std::flush;
}
...
AVM_ENDIF_DEBUG_FLAG( TIME )
		...
		if( mTrigger.valid() )
		{
			if( ENV.eval(anEC.getExecutionData(),
					anEC.getExecutionData().getSystemRID(), mTrigger) )
			{
		...
		const BF & aFormula, FormulaData * aFD)
{
	BF condition = ExpressionConstructor::andExpr(
			anEC.getExecutionData().getPathCondition(), aFormula);
...
AVM_ENDIF_DEBUG_FLAG( PROCESSOR )
		...
		anEC.getUniqInformation()
				->getUniqSpecificInfo< FormulaCoverageFilterInfo >()
		...
		{
			anEC.getwExecutionData().setAEES( AEES_STMNT_EXIT );
		}
...
{
	std::size_t succesCount = 0;

	for( auto & itChildEC : anEC.getChildContexts() )
	{
		if( ENV.evalFormula(*itChildEC, &mLocalExecutableForm, aFD->formula) )
		{
			if( checksatFormula(*itChildEC, ENV.outVAL, aFD) )
			{
				++succesCount;
...
{
	FormulaData * itFD;
	BFVector::const_iterator itForm = mFormulaTable.begin();
	BFVector::const_iterator endForm = mFormulaTable.end();
	for( ; itForm != endForm ; ++itForm )
	{
		itFD = (*itForm).to_ptr< FormulaData >(); // @suppress("Symbol is not resolved")
	...
	switch( aFD->formula.classKind() )
	{
		case FORM_AVMCODE_KIND:
		{
			...
			switch( aFormula->getAvmOpCode() )
			{
				case AVM_OPCODE_OBS :
				{
					const BFCode & bfFormula = aFormula->first().bfCode();
					...
					switch( evtFormula.getAvmOpCode() )
					{
						...
						case AVM_OPCODE_OUTPUT :
						{
							if( evtFormula.first().is< BaseInstanceForm >() )
							{
								...
								if( ioInstance.is< InstanceOfPort >() )
								{
									...
									for( auto & itChildEC : anEC.getChildContexts() )
									{
										apED = itChildEC->getExecutionData();

										BFCode ioTrace = ENV.searchTraceIO(
												itChildEC->getIOElementTrace(),
												evtFormula);
								...
								else if( ioInstance.is< InstanceOfData >() )
								{
									for( auto & itChildEC : anEC.getChildContexts() )
									{
										...
										{
											ENV.eval(apED, apED.getSystemRID(),
													constraintFormula);
						...
						default:
						{
							for( auto & itChildEC : anEC.getChildContexts() )
							{
								...
								ENV.eval(apED, apED.getSystemRID(), bfFormula);
...
AVM_IF_DEBUG_LEVEL_FLAG( MEDIUM , SOLVING )
	AVM_OS_TRACE << " ==> EC<" << itChildEC->getIdNumber()
...
bool FormulaCoverageFilter::checksatMoc(const BF & aMoc)
{
	switch( aMoc.classKind() )
	{
		...
		case FORM_EXEC_FILTER_FORMULA_DATA_KIND:
		{
			return( mFormulaTruthTable[ aMoc.to< FormulaData >().offset ] );
		}
...
void FormulaCoverageFilter::mocAppendFormulaLink(const BF & aMoc)
{
	switch( aMoc.classKind() )
	{
		case FORM_AVMCODE_KIND:
		{
			for( const auto & itOperand : aMoc.to< AvmCode >().getOperands() )
			{
				mocAppendFormulaLink( itOperand );
			}
			break;
		}
		...
		case FORM_EXEC_FILTER_FORMULA_DATA_KIND:
		{
			(mFormulaLinkTable[ aMoc.to< FormulaData >().offset ])++;
			break;
		}
...
void FormulaCoverageFilter::mocRemoveFormulaLink(const BF & aMoc)
{
	switch( aMoc.classKind() )
	{
		case FORM_AVMCODE_KIND:
		{
			for( const auto & itOperand : aMoc.to< AvmCode >().getOperands() )
			{
				mocRemoveFormulaLink( itOperand );
			}
			break;
		}
...
void FormulaCoverageFilter::mocUpdateUncoveredFormula()
{
	...
	for( ; itForm != mListOfUncoveredFormula.end() ; )
	{
		if( mFormulaLinkTable[ (*itForm).to< FormulaData >().offset ] < 0 )
		{
			itForm = mListOfUncoveredFormula.erase(itForm);
		}
		else
...
void FormulaCoverageFilterInfo::toStream(OutStream & os)  const
{
	if( mFormulaTable.nonempty() )
	{
		...
		for( ; itForm != endForm ; ++itForm )
		{
			(*itForm).to< FormulaData >().toStream(os);
		}
...
void FormulaCoverageFilterInfo::toFscn(OutStream & os) const
{
	if( mFormulaTable.nonempty() )
	{
		...
		for( ; itForm != endForm ; ++itForm )
		{
			(*itForm).to< FormulaData >().toFscn(os);
		}
