...
Symbol BaseEnvironment::create(
		const RuntimeID & aRID, const std::string & newfreshParamID,
		const BaseTypeSpecifier & aTypeSpecifier, const BF & aValue)
{
	...
	if( getConfiguration().isNewfreshParameterNameBasedPID() )
	{
		ossUfi << aRID.strPid() << ":" << newfreshParamID;
...
//				<< getConfiguration().getNameIdSeparator() << ++NEW_PARAM_OFFSET;
	...
	theSymbolicParam.setCreatorContainerRID( aRID );
	theSymbolicParam.setNameID( newfreshParamID );
					...
					case IPointerVariableNature::POINTER_FIELD_ARRAY_INDEX_NATURE:
					{
						...
						if( eENV.outVAL.isNumeric() )
						{
							symbolicIndex = new InstanceOfData(
									IPointerVariableNature::
											POINTER_FIELD_ARRAY_OFFSET_NATURE,
									(*it).getContainer(), (*it).getAstElement(), // @suppress("Method cannot be resolved")
									(*it).getTypeSpecifier(),
 ...
 * TOOLS
 */
const BFCode & BaseEnvironment::searchTraceIO(const BF & aTrace)
{
	if( aTrace.valid() )
	{
		if( aTrace.is< AvmCode >() )
		{
			...
			{
				const BFCode & ioTrace = searchTraceIO(itOperand);
				if( ioTrace.valid() )
				{
					return( ioTrace );
				}
		...
		else if( aTrace.is< ExecutionConfiguration >() )
		{
			...
			{
				const BFCode & ioTrace = execConf.getAvmCode();
...
const BFCode & BaseEnvironment::searchTraceIO(
		const BF & aTrace, const AvmCode & ioFormula)
{
	if( aTrace.valid() )
	{
		if( aTrace.is< AvmCode >() )
		{
			for( const auto & itOperand : aTrace.to< AvmCode >().getOperands() )
			{
				const BFCode & ioTrace = searchTraceIO(itOperand, ioFormula); // @suppress("Invalid arguments")
				if( ioTrace.valid() )
				{
					return( ioTrace );
				}
		...
		const RuntimeID & ctxRID, const AvmCode & ioFormula)
{
	if( aTrace.valid() )
	{
		if( aTrace.is< AvmCode >() )
		{
			for( const auto & itOperand : aTrace.to< AvmCode >().getOperands() )
			{
				const BFCode & ioTrace = searchTraceIO(itOperand, ctxRID, ioFormula);
				if( ioTrace.valid() )
				{
					return( ioTrace );
				}
