...
public:
	...
	WaitingStrategyBLOCK(
			Configuration & aConfiguration, std::uint8_t queueCount,
	...
	virtual void push(ExecutionContext * anEC) override
	{
	...
	virtual ExecutionContext * top() override
	{
...
protected:
	/*ATTRIBUTES*/
	std::size_t mBlockHeightPeriod;
	std::size_t mBlockWidthPeriod;

	std::size_t mBlockHeight;
	std::size_t mBlockWidth;

	std::size_t mHeightLimit;
	std::size_t mWidthLimit;

	std::size_t mPushHeightMin;
	std::size_t mPushWidthMin;

	std::size_t mPushHeightMax;
	std::size_t mPushWidthMax;
...
public:
	...
	virtual ExecutionContext * pop() override
	{
		return( nullptr );
	}
	...
	{
		static std::size_t call_counter = 0;
		++call_counter;
			...
			while( true )
			{
						...
						(base_this_type::mBlockWidth < base_this_type::mPushWidthMax) )
				{
					...
					AVM_OS_COUT << "update mBlockWidth [" << call_counter << "] :> "
							...
							<< base_this_type::mPushHeightMin << " , "
							<< base_this_type::mPushHeightMax << " ]"
							<< " => height:" << base_this_type::mBlockHeight << " <---> "
							<< "W<" << base_this_type::mBlockWidthPeriod << ">[ "
							<< base_this_type::mPushWidthMin << " , "
							<< base_this_type::mPushWidthMax << " ]"
