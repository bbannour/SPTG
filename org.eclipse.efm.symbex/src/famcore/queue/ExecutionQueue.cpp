...
bool ExecutionQueue::configureImpl()
{
	...
	const WObject * wfQueue = mParameterWObject;
	...
	const WObject * theQUEUE = ( isnotDefinedWTypedObjectParameters ) ?  wfQueue
	...
	if( theQUEUE != WObject::_NULL_ )
	{
		...
		std::string strStrategy = ( theQUEUE->isWSequence() )
				? Query::getRegexWPropertyString(
						theQUEUE, OR_WID2("strategy", "queue"), "BFS")
				: theQUEUE->strValue();
			...
			REGEX_FIND(strStrategy, CONS_WID3("DEPTH", "FIRST", "SEARCH")) )
		{
			...
			REGEX_FIND(strStrategy, CONS_WID3("RANDOM", "FIRST", "SEARCH")) )
		{
		...
		if( strStrategy.find("BLOCK") != std::string::npos )
		{
			...
			std::size_t aBlockHeightPeriod;
			std::size_t aBlockWidthPeriod;
			std::size_t aBlockHeight;
			std::size_t aBlockWidth;

			std::size_t aHeightLimit;
			std::size_t aWidthLimit;
...
AVM_ENDIF_DEBUG_FLAG( CONFIGURING )
			...
			{
				configureWaitingWeightStrategyAll((std::uint8_t) anInteger);
			}
			else
			{
				configureWaitingWeightStrategy((std::uint8_t) anInteger);
			}
		}
...
void ExecutionQueue::configureWaitingWeightStrategy(std::uint8_t maxWeight)
{
	switch( mStrategy & STRATEGY_FAMILY_BASIC )
	{
		case STRATEGY_BFS:
...
void ExecutionQueue::configureWaitingWeightStrategyAll(std::uint8_t maxWeight)
{
	switch( mStrategy & STRATEGY_FAMILY_BASIC_ALL )
	{
		case STRATEGY_BFS:
...
bool ExecutionQueue::reconfigure(
		ENUM_STRATEGY_T newStrategy, std::uint8_t queueCount)
{
	if( (mStrategy == newStrategy) && (mWaitingStrategy != nullptr) )
	{
		return( true );
	}
...
void ExecutionQueue::toStream(OutStream & os) const
{
	...
	if( mAnteWaitingQueue.nonempty() )
	{
		os << TAB << "ante waiting< size: " << mAnteWaitingQueue.size() << " >:"
				<< EOL_INCR_INDENT;

		toStream(mAnteWaitingQueue, os);
		os << DECR_INDENT;
	}
		...
		const ListOfExecutionContext & aQueue, OutStream & os) const
{
	for( const auto & itQueueEC : aQueue )
	{
		if( (itQueueEC != nullptr) && itQueueEC->isAlive() )
		{
			itQueueEC->traceDefault(os);
		}
