		...
		const ExecutionContext & anEC, const std::string & strFlag)
{
	...
	if( StringTools::startsWith(strFlag, "INFO") )
	{
		isMatch = anEC.hasInfo();
	}

	if( StringTools::startsWith(strFlag, "DEADELOCK") )
	{
		isMatch = anEC.getFlags().isExecutionDeadlockTrace();
	}
	else if( StringTools::startsWith(strFlag, "LIVELOCK") )
	{
	...
	else if( StringTools::startsWith(strFlag, "REDUNDANCY") )
	{
	...
	else if( StringTools::startsWith(strFlag, "COVERAGE") )
	{
