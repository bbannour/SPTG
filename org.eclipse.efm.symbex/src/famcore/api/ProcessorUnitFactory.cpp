...
static bool isMainProcessorUnitTypeID(const WObject & wfObject)
{
	return( MainProcessorUnit::AUTO_REGISTER_TOOL.isTypeID(wfObject) );
}

static bool isExecutionQueueTypeID(const WObject & wfObject)
{
	return( ExecutionQueue::AUTO_REGISTER_TOOL.isTypeID(wfObject) );
}


bool ProcessorUnitFactory::configure(
		SymbexControllerUnitManager & aPluginProcessorManager,
		const WObject * wfDirectorObject, const WObject * moeProfile)
{
	/////////////////////////////////////
	// PROCESSORS CREATION
	/////////////////////////////////////
	...
	Query::getRegexWObjectInSequence(wfDirectorObject,
			WorkflowParameter::SECTION_FAM_CONTAINERS_REGEX_ID, listOfProcessWObject);
...
*/
	...
	if( not theMainProcessor.hasParameterWObject() )
	{
		theMainProcessor.setParameterWObject(
				Query::getWObject(wfDirectorObject, isMainProcessorUnitTypeID) );
	}
	...
	const WObject * queueWObject = Query::getRegexWObjectByNameID(
			theMainProcessor.getParameterWObject(),
			WorkflowParameter::SECTION_FAM_QUEUE_REGEX_ID, WObject::_NULL_);
	...
	else
	{
		...
		if( queueWObject == WObject::_NULL_ )
		{
			...
			{
				queueWObject =
						Query::getWObject(moeProfile, isExecutionQueueTypeID);
			}
		}
	}
		...
		CompositeControllerUnit & processorScheduler, const BF & aScheduler)
{
	if( aScheduler.valid() )
	{
		AbstractProcessorUnit * aProcessor = nullptr;
		...
		{
			const AvmCode & aCode = aScheduler.to< AvmCode >();

			for( const auto & itOperand : aCode.getOperands() )
			{
				configureProcessorScheduler(
						aRequirement, processorScheduler, itOperand);
			}
		}
		else
		{
