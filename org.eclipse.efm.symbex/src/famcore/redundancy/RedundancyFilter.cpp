...
bool RedundancyFilter::configureImpl()
{
	...
	const WObject * wfRedundancy = mParameterWObject;
	...
	const WObject * thePROPERTY = (isnotDefinedWTypedObjectParameters)
			? wfRedundancy
	...
	if( (path_scope == "ALL") || (path_scope == "ANY") )
	{
		mPathScopeIterator =
				new AllPathScopeIterator(mConfigurationComparator);
	}
	else if( (path_scope == "CURRENT") || (path_scope == "ANCESTOR"))
	{
		mPathScopeIterator =
				new CurrentPathScopeIterator(mConfigurationComparator);
	}
	else if( (path_scope == "PARENT") || (path_scope == "CONTAINER"))
	{
		mPathScopeIterator =
	...
	else if( mRedundancyEnabledFlag )
	{
		...
		for( ; mPathScopeIterator->hasNext() ; mPathScopeIterator->next() )
		{
					...
					anEC, (* mCandidateEC)) )
			{
				...
				BF ecHeader = BF( mCandidateEC->rawHeader() );
				anEC.addInfo(*this, ecHeader);
...
AVM_IF_DEBUG_LEVEL_OR_FLAG( MEDIUM , REDUNDANCE )
	AVM_OS_INFO << std::endl << DEFAULT_WRAP_DATA << "REDUNDANT EC :> ";
	anEC.traceDefault(AVM_OS_INFO);

	AVM_OS_INFO << "FOUND[" << std::setw(5)
			<< mRedundancyCount << "] :> ";
	mCandidateEC->traceDefault(AVM_OS_INFO);
	AVM_OS_INFO << END_WRAP_EOL;
...
AVM_IF_DEBUG_LEVEL_FLAG( MEDIUM , REDUNDANCE )
		AVM_OS_INFO << DEFAULT_WRAP_DATA << "NEW EC :> ";
	anEC.traceDefault(AVM_OS_INFO);
...
bool RedundancyFilter::trivialLoopDetection(ExecutionContext & anEC)
{
	...
	{
		for( ; (prevEC != nullptr) && prevEC->isnotRoot() ;
				prevEC = prevEC->getPrevious()  )
		{
			if( anEC.edTEQ( *prevEC ) )
			{
				++mTrivialLoopCount;
	...
	else if( _AVM_EXEC_MODE_ != AVM_EXEC_SERVER_GRPC_MODE )
	{
		if( (prevEC != nullptr) && prevEC->isnotRoot() && anEC.edTEQ( *prevEC ) )
		{
			++mTrivialLoopCount;
	...
	if( foondLoop )
	{
		...
		// DO DIAGNOSTIQ
AVM_IF_DEBUG_LEVEL_OR_FLAG( MEDIUM , REDUNDANCE )
	AVM_OS_INFO << std::endl << "TRIVIAL LOOP DETECTION :> ";
	anEC.traceDefault(AVM_OS_INFO);

	AVM_OS_INFO << "COUNT[" << std::setw(5) << mTrivialLoopCount << "] :> ";
	prevEC->traceDefault(AVM_OS_INFO);
	AVM_OS_INFO << std::endl;
AVM_ENDIF_DEBUG_LEVEL_OR_FLAG( MEDIUM , REDUNDANCE )
...
void RedundancyFilter::tddRegressionReportImpl(OutStream & os) const
{
