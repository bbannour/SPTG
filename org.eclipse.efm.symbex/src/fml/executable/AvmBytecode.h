...
//typedef std::uint8_t         avm_arg_context_t;
...
//typedef std::uint8_t         avm_arg_processor_t;
...
//typedef std::uint8_t           avm_arg_operation_t;
...
#define IS_ARG_OPERATION_NOP_ALL(op)   (op & AVM_ARG_NOP_ALL)

#define IS_ARG_OPERATION_NOPS(op)      (op & AVM_ARG_NOPS)
...
//typedef std::uint8_t           avm_arg_operand_t;
...
//	}
	 ...
	 * result
	 */
	void set(avm_arg_context_t aContext, avm_arg_processor_t aProcessor,
			avm_arg_operation_t anOperation, avm_arg_operand_t anOperand)
	{
		context   = aContext;
			...
			avm_arg_operation_t opMask = AVM_ARG_UNDEFINED_OPERATION)
	{
		...
		setNopOperation( opMask );
	}

	inline void setNopsCpu(
			avm_arg_operation_t opMask = AVM_ARG_UNDEFINED_OPERATION)
	{
		processor = AVM_ARG_NOPS_CPU;

		setNopsOperation( opMask );
	}
	...
	inline bool isNopAllOperation() const
	{
		return( IS_ARG_OPERATION_NOP_ALL(operation) );
	}

	inline void unsetNopAllOperation(
			avm_arg_operation_t opMask = AVM_ARG_UNDEFINED_OPERATION)
	{
