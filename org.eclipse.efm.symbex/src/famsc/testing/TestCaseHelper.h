...
protected:
	...
	static bool mSequenceCoverage;

	static bool mDeterministic;

	static bool mTimedModel;

	static TraceFilter mProjection;

	static BFList mListOfInobservableDelays;

	ListOfExecutionContext mListOfECWithOutputPort;
...
//	BFList getParamsListOfPortForInternal(const BF & ioTrace);
	...
	BF constructPtCwithoutInobservableDelays(
			BF & newAtomicFormula, const BF & pathTimedCondition );
...
//	const void getIOPortParameter(InstanceOfPort * aPort, const BF & ioTrace, BFList & paramsList);
	...
	void verifyTreatedOutputPortForFail(Vector<InstanceOfPort *> & listOfPorts,
			InstanceOfPort * aPort);

	void verifyTreatedInputPortForInconc(Vector<InstanceOfPort *> & listOfPorts,
			InstanceOfPort * aPort);

	void getListOfOutputPorts(InstanceOfPort * aPort,
			Vector<InstanceOfPort *> & listOfPorts);

	void getListOfInputPorts(InstanceOfPort * aPort,
			Vector<InstanceOfPort *> & listOfPorts);
	...
	BFCode resettingClockTestCase( const BF & clockTestCase );

	BFCode readingValueOfClockTestCase(const BF & clockTestCase, BF delay);
