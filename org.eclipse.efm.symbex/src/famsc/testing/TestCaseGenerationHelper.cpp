...
#include "TestCaseGenerationHelper.h"
...
InstanceOfPort * TestCaseGenerationHelper::getIOPort(const BF & ioTrace)
{
	if( ioTrace.invalid() )
	{
		return( nullptr );
...
const RuntimeID & TestCaseGenerationHelper::getIORuntimeID(const BF & ioTrace)
{
	if( ioTrace.invalid() )
	{
...
InstanceOfPort * TestCaseGenerationHelper::getIOPort(const BF & ioTrace, RuntimeID & aRID)
{
	if( ioTrace.invalid() )
	{
...
void TestCaseGenerationHelper::keepDelayOfTransition(const BF & ioTrace, BFList & newFreshTrace)
{
	if ( ioTrace.invalid() )
	{
...
BF TestCaseGenerationHelper::getDelayOfTransition(const BF & ioTrace)
{
	BF delay;
	...
	{
//		AVM_OS_DEBUG << "1 " <<  std::endl;//TODO
		const ExecutionConfiguration & aConf = ioTrace.to< ExecutionConfiguration >();
//		AVM_OS_DEBUG << "2 " <<  std::endl;//TODO

		if( aConf.getCode().is< AvmCode >() )
		{
//			AVM_OS_DEBUG << "3" <<  std::endl;//TODO
			const AvmCode & ioCode = aConf.getCode().to< AvmCode >();
//			AVM_OS_DEBUG << "4" <<  std::endl;//TODO

//			AVM_OS_DEBUG << "ioTrace.toStream() : " <<  std::endl;//TODO
//			ioTrace.toStream(AVM_OS_DEBUG) ;

			if( ioCode.second().is< InstanceOfData >() )
			{
//				AVM_OS_DEBUG << "5" <<  std::endl;//TODO
				delay = ioCode.second();
//				AVM_OS_DEBUG << "6" <<  std::endl;//TODO
			}
		}
	}
	else if( ioTrace.is< AvmCode >() )
	{
		...
		for( const auto & itOperand : aCode.getOperands() )
		{
			if ( (itOperand.to< ExecutionConfiguration >().getCode().to< AvmCode >()).first().is< InstanceOfData >() )
			{
				delay = getDelayOfTransition( itOperand );
				return delay;
			}
		}
	}
...
BFList TestCaseGenerationHelper::getParamsListOfPort(const BF & ioTrace)
{
	BFList paramsList;
	if( ioTrace.invalid() )
	{
...
void TestCaseGenerationHelper::getTimeVarsNonInstantiatedInGuard(const BF & bigFormula,
		AvmCode::OperandCollectionT & listOfVarsNonInstantiated)
{
	...
	{
//		AVM_OS_DEBUG << "bigFormula is not an EXPRESSION !" << std::endl;//TODO
	}
}

void TestCaseGenerationHelper::getDataVarsNonInstantiatedInGuard(const BF & bigFormula,
		AvmCode::OperandCollectionT & listOfVarsNonInstantiated)
{
	bool newVar = true;
...
BFList TestCaseGenerationHelper::getBSenInQuestion(const BFList & BSen,
		AvmCode::OperandCollectionT & listOfVarsNonInstantiated)
{
	BFList bSenFormulas;
...
AvmCode::OperandCollectionT TestCaseGenerationHelper::getTermListOfPort(const BF & ioTrace)
{
...
void TestCaseGenerationHelper::setIOMirroring(ExecutionContext & anEC)
{
...
BF TestCaseGenerationHelper::getIOMirroring(const BF & ioTrace)
{
	if( ioTrace.invalid() )
	{
		return( BF::REF_NULL );
...
void TestCaseGenerationHelper::verifyIncontrollablePath(ExecutionContext * childEC)
{
...
void  TestCaseGenerationHelper::getListOfOutputPorts(
...
void TestCaseGenerationHelper::verifyTreatedPortForFail(
...
BF TestCaseGenerationHelper::createNotEqualityOnFreshVars(
...
BF TestCaseGenerationHelper::createEqualityOnFreshVarsForGraph(
...
BF TestCaseGenerationHelper::createEqualityOnFreshVars(
		ExecutionContext * anEC, AvmCode::OperandCollectionT newfreshList, BFList & listInstDataVars )
{
	BFList paramsList = getParamsListOfPort(anEC->getIOElementTrace());
...
//
//	AVM_OS_DEBUG << "pChildECPass : " << pChildECPass->str() << " node condtion : " <<pChildECPass->getNodeCondition() << std::endl;//TODO
//
...
void TestCaseGenerationHelper::addRunnableTrace(ExecutionContext * childEC, const RuntimeID & compRID)
{
	BFList runTrace;
...
void TestCaseGenerationHelper::createQuiescenceInGraph(ExecutionContext * childEC,
		ExecutionContext * parentEC, ListOfExecutionContext & listofECToAdd)
{
...
BFCode TestCaseGenerationHelper::resettingClockTestCase(Variable * clockTestCase)
{
	BFCode resettingClock = StatementConstructor::newCode(
			OperatorManager::OPERATOR_ASSIGN, BF(clockTestCase),
			ExpressionConstant::INTEGER_ZERO );

//	AVM_OS_DEBUG << "resettingClock : " << resettingClock.str() << std::endl;//TODO

	//	newtran->setStatement( incrCurrentTime ) ;

	return resettingClock;
}

BFCode TestCaseGenerationHelper::readingValueOfClockTestCase(Variable * clockTestCase, BF delay)
{
	BFCode readingClock = StatementConstructor::newCode(
			OperatorManager::OPERATOR_ASSIGN, delay, BF(clockTestCase) );
