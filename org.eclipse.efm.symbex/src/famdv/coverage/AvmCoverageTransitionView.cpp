		...
		switch( mScope )
		{
			...
			default: // for  << @scope = 'DETAILS'; >>
			{
				...
				if( theDETAILS != WObject::_NULL_ )
				{
					...
					// INITIALISATION de la table de tous les PROGRAM
					// associés à chacune des INSTANCE...
					for( const auto & itInst : listOfInstance )
					{
						if( itInst->refExecutable().hasTransition() )
						{
...
AVM_IF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
	AVM_OS_TRACE << "\t" << "program :> "
			<< aRID.refExecutable().rawTransition(itTransition)->getFullyQualifiedNameID()
			<< std::endl;
AVM_ENDIF_DEBUG_LEVEL_FLAG( MEDIUM , CONFIGURING )
					}
				}
...
void AvmCoverageTransitionView::reportCoverageStateTransitions(OutStream & os,
...
//os << TAB3 << "transitions : uncovered count = " << uncoveredCount << EOL_FLUSH;
	...
	if( uncoveredCount > 0 )
	{
		...
		{
			os << TAB3 << nature << "[ NO INCOMING TRANSITION ] :> " << stateID << EOL;
		}

		AvmTransition * itTransition = nullptr;
		std::size_t endOffset = anExecutable.getTransition().size();

		if( uncoveredCount > 1 )
		{
			os << TAB3 << "transitions of " << stateID << " {" << EOL;
			for( std::size_t offset = 0 ; offset < endOffset ; ++offset )
			{

				if( not coverageTable->test( offset ) )
				{
					itTransition = anExecutable.getTransition().rawAt(offset);

					os << TAB4 << itTransition->getNameID() << " --> "
							<< itTransition->getTarget().str() << EOL;
				}
			}
			os << TAB3 << "}" << EOL;
		}
		else
		{
			for( std::size_t offset = 0 ; offset < endOffset ; ++offset )
			{
				if( not coverageTable->test( offset ) )
				{
					...
					os << TAB3 << "transition of " << stateID << " : "
							<< itTransition->getNameID() << " --> "
							<< itTransition->getTarget().str() << EOL;
					break;
				}
			}
			...
			switch( mScope )
			{
				case Specifier::DESIGN_INSTANCE_KIND:
				{
					...
					for( ; itMachine < endMachine ; ++itMachine )
					{
						...
						reportCoverageStateTransitions(os,
								mTransitionCoverageTable->at(aRID.getOffset()),
								aRID.refExecutable(),
...
bool AvmCoverageTransitionView::prefiltering(ListOfExecutionContext & ecQueue)
{
	...
	for( itQueue = ecQueue.begin() ; itQueue != endQueue ; ++itQueue )
	{
		...
		else if( (mDirectiveHitEC == nullptr) && (*itQueue)->
...
AVM_ENDIF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	...
	{
AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	AVM_OS_INFO  << "==> Next [ Certainly ] Fireable Count :> "
...
AVM_IF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , QUEUE )
	AVM_OS_INFO << "AvmCoverageTransitionView::highRequeueWaitingTable :>" << std::endl;
...
AVM_ENDIF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
		...
		for( const auto & itQueueEC : mWaitingQueue )
		{
			computeWeight( itQueueEC );

			aWaitingStrategy.push( itQueueEC );

AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	if( itQueueEC->getWeight() <= WEIGHT_CERTAINLY_ACHIEVABLE )
	{
		AVM_OS_INFO  << DEFAULT_WRAP_DATA << "REQUEUE candidate [ <= Certainly ] :> ";

		itQueueEC->traceMinimum(AVM_OS_COUT); AVM_OS_COUT << END_WRAP;
		fireableTransitionTrace(AVM_OS_COUT, itQueueEC->getExecutionData());

		itQueueEC->traceMinimum(AVM_OS_TRACE); AVM_OS_TRACE << END_WRAP;
		fireableTransitionTrace(AVM_OS_TRACE, itQueueEC->getExecutionData());
	}

	else if( itQueueEC->getWeight() <= WEIGHT_STRONGLY_ACHIEVABLE )
	{
		AVM_OS_INFO  << DEFAULT_WRAP_DATA << "REQUEUE candidate [ <= Strongly ] :> ";
...
AVM_ENDIF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , QUEUE )
		...
		{
AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	AVM_OS_INFO  << "==> REQUEUE candidate [ Certainly ] Fireable Count :> "
			<< mCertainlyFireableTransitionCount
			<< "  Hit Count : " << mCertainlyHitEC.size() << std::endl;
AVM_ENDIF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
		}
...
AVM_IF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , QUEUE )
	AVM_OS_INFO << "AvmCoverageTransitionView::elseRequeueWaitingTable :>" << std::endl;
	aWaitingStrategy.toStream(AVM_OS_COUT);
	aWaitingStrategy.toStream(AVM_OS_TRACE);
AVM_ENDIF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , QUEUE )
...
AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	if( itQueueEC->getWeight() <= WEIGHT_CERTAINLY_ACHIEVABLE )
	{
		AVM_OS_INFO  << "REQUEUE candidate [ <= Certainly ] :> ";

		itQueueEC->traceMinimum(AVM_OS_COUT);
		fireableTransitionTrace(AVM_OS_COUT, itQueueEC->getExecutionData());

		itQueueEC->traceMinimum(AVM_OS_TRACE);
		fireableTransitionTrace(AVM_OS_TRACE, itQueueEC->getExecutionData());
	}

	else if( itQueueEC->getWeight() <= WEIGHT_STRONGLY_ACHIEVABLE )
	{
		AVM_OS_INFO  << "REQUEUE candidate [ <= Strongly ] :> ";
		...
		const ListOfExecutionContext & ecQueue)
{
	...
	for( const auto & itQueueEC : ecQueue )
	{
		endEC = itQueueEC->end();
		for( itEC = itQueueEC->begin() ; (itEC != endEC) ; ++itEC )
		{
			computeWeight( *itEC );
...
std::uint8_t AvmCoverageTransitionView::checkFireability(
		const ExecutionData & anED, const RuntimeID & aRID,
		AvmTransition * aTransition)
{
	if( aTransition->hasCode() && aTransition->getCode()->hasOperand()
...
AVM_IF_DEBUG_LEVEL_FLAG2( HIGH , PROCESSOR , TRANSITION )
	AVM_OS_INFO  << "==> Fireable [ Certainly ] :> "
...
{
	for( tmpEC = anEC->getPrevious() ; tmpEC != nullptr ; tmpEC = tmpEC->getPrevious() )
	{
...
//					}
//
//					anEC->addInfo(mCoverageProcessor, INCR_BF( firedTransition ) );
//
...
{
	std::size_t endTransition = aTableofUncoveredTransition.size();
	std::size_t uncoveredOffset = 0;
...
AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
	AVM_OS_INFO  << "AvmCoverageTransitionView::updateUncoveredTransition:> count: "
			<< aTableofUncoveredTransition.size() << std::endl;

	for( const auto & itTransition : aTableofUncoveredTransition )
	{
...
//					for( offset = 0 ; offset < endOffset ; ++offset )
//					{
//						itTransition = aRID.refExecutable().getTransition().rawAt(offset);
//
//						if( not mTransitionCoverageTable->
...
//endComputing:
//AVM_IF_DEBUG_LEVEL_FLAG2( MEDIUM , PROCESSOR , TRANSITION )
//	AVM_OS_INFO  << "AvmCoverageTransitionView::collectUncoveredTransition:> count: "
//			<< aTableofUncoveredTransition.size() << std::endl;
//
					...
					if( aRID.refExecutable().hasTransition() )
					{
						...
						for( offset = 0 ; offset < endOffset ; ++offset )
						{
							...
							if( (not mTransitionCoverageTable->
									at(aRID.getOffset())->test(offset))
								&& mExecutableCoverageTable->
									at(aRID.refExecutable().getOffset())->
									test(offset) )
							{
								++uncoveredCount;
