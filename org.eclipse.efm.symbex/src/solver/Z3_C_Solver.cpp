...
Z3_sort Z3Solver::getZ3Type(const BaseTypeSpecifier & bts)
{
...
//		<< "(get-info :version)" << std::endl;
	...
	for( std::size_t offset = 0 ; offset < paramCount ; offset++ )
	{
		...
		else if( paramVector[ offset ].is< Variable >() )
		{
			...
			{
				os << "(assert (< 0 " << uniqVariableID(aVariable) <<"))"
					<< std::endl;
			}
		...
		{
			os << "\t(" << paramVector[ offset ].str() << ")" << std::endl;
		}
	}
			...
			if( hasQuantifier )
			{
				...
				for( ; (itOperand + 1) != endOperand ; ++itOperand )
				{
					...
					else if( (*itOperand).is< Variable >() )
					{
						...
						os << "\t(" << uniqVariableID(aVariable)
							<< " " << varType << ")" << std::endl;

						if( aVariable.getTypeSpecifier().isTypedStrictlyPositiveNumber() )
						{
							++termCount;
							typeConstraint << "(< 0 "
