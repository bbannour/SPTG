...
for bench_dir in */; do
	if [ -d "$bench_dir" ];
	then
		BENCH_PATH="$( realpath "$bench_dir" )"

		if [ -f $BENCH_PATH/$RUN_SAMPLE_ALL_SH ]
		then
			if [ ! -x $BENCH_PATH/$RUN_SAMPLE_ALL_SH ]
			then
				chmod a+x $BENCH_PATH/$RUN_SAMPLE_ALL_SH
			fi

			echo "____________________________________________________________"
			echo "Eval $BENCH_PATH/$RUN_SAMPLE_ALL_SH"
			$BENCH_PATH/$RUN_SAMPLE_ALL_SH
			...
			then
				...
				ec# Ensure that we are in the directory of this example "Exit code : $RUN_SAMPLE_ALL_SH_RETURN_CODE"
...
echo "| End SPTG on all benchmarks !"
echo "____________________________________________________________"
